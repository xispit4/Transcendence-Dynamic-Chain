...
if [ $EE = "2" ] 
  ...
  COUNTER=$((COUNTER+1))
	...
	echo "alias ${ALIAS}_reindex=\"systemctl stop transcendenced$ALIAS && /root/bin/transcendenced_${ALIAS}.sh -reindex && echo -e "${GREEN}Restart wallet after reindex is completed${NC}"\"" >> .bashrc
